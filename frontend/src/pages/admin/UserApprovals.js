...
const UserApprovals = () => {
  ...
  const fetchPendingApprovals = async () => {
    try {
      ...
      const response = await axios.get('http://localhost:5000/api/admin/approvals/pending', {
  ...
  const fetchStatistics = async () => {
    try {
      const token = localStorage.getItem('token');
      const response = await axios.get('http://localhost:5000/api/admin/approvals/statistics', {
        headers: { Authorization: `Bearer ${token}` }
      });
      ...
      <div className="mb-4 p-3 bg-light rounded">
        ...
        <div className="row">
          {Object.entries(statistics).map(([userType, stats]) => (
            <div key={userType} className="col-md-4">
