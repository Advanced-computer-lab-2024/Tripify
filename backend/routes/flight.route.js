...
const getAccessToken = async () => {
  try {
    ...
    if (response.data && response.data.access_token) {
      ...
      tokenExpiration = new Date(new Date().getTime() + (response.data.expires_in * 1000));
    ...
    if (error.response?.status === 400) {
      ...
      res.status(404).json({
        errors: [{
          ...
          detail: 'The selected flight is no longer available. Please search for new flights.'
        }]
      });
    } else {
